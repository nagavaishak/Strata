...
//
// Usage: ANCHOR_WALLET=~/.config/solana/oracle-keypair.json ts-node -P tsconfig.json scripts/live-buyer-flow.ts <fixtureId>
...
async function authedHttp(program: anchor.Program, keypair: Keypair) {
  const SUBSCRIPTION_MINT = new PublicKey("4Zao8ocPhmMgq7PdsYWyxvqySMGx7xb9cMftPMkEokRG");
  const { getAssociatedTokenAddressSync, createAssociatedTokenAccountIdempotentInstruction, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID } = await import("@solana/spl-token");
  ...
  const txoracleProgram = new anchor.Program(txoracleIdl as anchor.Idl, program.provider);
...
async function fetchLatestForFixture(http: any, fixtureId: number, statKey: number, sinceMs?: number): Promise<any | null> {
  for (let back = 1; back <= 24; back++) {
    ...
    // API returns PascalCase fields (FixtureId, Seq), not the camelCase used elsewhere in
    // this codebase's REST responses (e.g. stat-validation's own response body).
    const matches = list.filter((u) => (u.FixtureId ?? u.fixtureId ?? u.fixture_id) === fixtureId);
    if (!matches.length) continue;
    // Take the highest seq in this bucket — the freshest update for our fixture.
...
async function main() {
  ...
  const http = await authedHttp(program, writer);

  // 1. Confirm the fixture is producing data right now.
  console.log(`Checking fixture ${FIXTURE_ID} is live...`);
  let anyBatch: any = null;
  for (let i = 0; i < 10 && !anyBatch; i++) {
    anyBatch = await fetchLatestForFixture(http, FIXTURE_ID, STAT_KEY);
    if (!anyBatch) {
      console.log(`  not live yet, retry ${i + 1}/10 (30s interval)`);
      await sleep(30_000);
    }
  }
  ...
  let proof: any = null;
  for (let i = 0; i < 20 && !proof; i++) {
    proof = await fetchLatestForFixture(http, FIXTURE_ID, STAT_KEY, closesAtMs);
    if (!proof) {
      console.log(`  no fresh-enough batch yet, retry ${i + 1}/20 (30s interval)`);
      await sleep(30_000);
    }
  }
